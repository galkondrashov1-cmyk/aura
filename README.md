...
- `/admin` — סטטיסטיקות פלטפורמה וניהול עסקים (שינוי חבילה, השעיה/החזרה, מחיקה, חיפוש).
- כניסה בשני שלבים: התחברות רגילה של חשבון עם `role=ADMIN`, ואז קוד גישה ב־`/admin-gate`
  (ברירת מחדל `5246` — לשינוי בפרודקשן קובעים משתנה סביבה `ADMIN_CODE`).
- עסק מושעה מוחזר כ־404 בכל המשטחים הציבוריים (עמוד, זימון, APIs).
- ה־seed יוצר את חשבון האדמין של גל + עסק דמו (פרטים ב־`scripts/seed-demo.mjs`).
